...
class WithdrawalBDOExport implements FromCollection, WithHeadings, WithEvents
{   
    use Exportable;
    ...
    public function __construct(string $bdo, int $total)
    {
        $this->bdo = $bdo;
        $this->total = $total + 2;
    }
    ...
    public function registerEvents(): array
    {
        return [
            BeforeExport::class  => function(BeforeExport $event) {
                ...
                $event->writer->setTitle('BDO Payout - '.$this->bdo);
    ...
    {
    	$withdrawals = Withdrawal::with('user')->where('mode','BDO')->where('status','PENDING')->where('card_type',$this->bdo)->orderBy('created_at','desc')->get();
